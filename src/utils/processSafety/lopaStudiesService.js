// LOPA & SIL Studio persistence (Process Safety PS1): ps_lopa_studies.
//
// A LOPA study belongs to the ORGANIZATION, because a safety study is a
...
// 20260919210000_ps1_lopa_studies.sql); the organization filter here is the
...
export const LOPA_STUDIES_TABLE = 'ps_lopa_studies';
export const LOPA_STUDY_KIND = 'ps-lopa-study';
export const LOPA_STUDIES_MIGRATION = '20260919210000_ps1_lopa_studies';

registerStateKind(LOPA_STUDY_KIND, { current: 1, migrations: {}, label: 'LOPA study' });

export const NO_ORG_MESSAGE = 'LOPA studies are saved to your organization. Join or select an organization to save one.';

/**
 * @param {() => (string|null)} getOrgId the caller's current organization id
 */
export function createLopaStudiesService(getOrgId) {
  const orgId = () => {
    const id = getOrgId();
  ...
  return {
    ...
    async save(id, payload) {
      const org = orgId();
      const { error } = await writeStamped(LOPA_STUDY_KIND, {
        id,
        organization_id: org,
        name: (payload?.name || '').trim() || 'Untitled study',
        payload,
        updated_at: new Date().toISOString(),
      }, (row) => supabase.from(LOPA_STUDIES_TABLE).upsert(row));
      if (error) throw error;
      return { success: true };
    },
    ...
    async load(id) {
      ...
      if (error) throw error;
      if (!data) return null;
      const { row } = openState(LOPA_STUDY_KIND, data);
