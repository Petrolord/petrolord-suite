...
/**
 * AS10 — one finding, to closure.
 *
 * The closure rule is AS8's, imported: correction, root cause, and for
 * a major nonconformity a corrective action verified effective. The
 * one thing this page adds is the stop-work record, which exists from
 * the moment the finding was raised rather than being written here.
 */
...
export default function FindingDetail() {
  ...
  const {
    findings, audits, responses, templateItems, activityFor,
  ...
  const response = useMemo(
    () => (finding?.response_id ? responses.find((r) => r.id === finding.response_id) || null : null),
    [responses, finding]);
  const item = useMemo(
    () => (response ? templateItems.find((i) => i.id === response.item_id) || null : null),
  ...
  if (loading) return <AuditShell><Loading label="Loading the finding..." /></AuditShell>;
  ...
  if (!finding) {
    return (
      <AuditShell title="Finding not found">
        <EmptyState
  ...
  return (
    ...
    >
      <div className="space-y-6 animate-in fade-in duration-300 pb-10">
        ...
        {finding.stop_work ? (
          <div className="p-4 rounded-xl border border-[hsl(var(--destructive))]/40 bg-[hsl(var(--destructive))]/5">
            <p className="font-medium text-[hsl(var(--destructive))]">Work was stopped for this finding.</p>
            <p className="text-sm mt-1">{finding.correction}</p>
          </div>
        ) : null}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="panel-elevation lg:col-span-1">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              {item ? (
                <DetailField label="Checklist item">
                  <span className="font-mono text-xs mr-2">{item.item_no}</span>
                  {item.question}
                </DetailField>
              ) : null}
              <div className="grid grid-cols-2 gap-4">
                <DetailField label="Site">{finding.site}</DetailField>
                ...
                <DetailField label="Closed">{finding.closed_date}</DetailField>
              </div>
              {audit ? (
            ...
            <CardHeader className="border-b border-[hsl(var(--border))] pb-4 flex flex-row items-start justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                  The correction deals with what was found; the root cause is what stops
                  it being found again at the next audit.
                </p>
              </div>
              {!terminal ? (
                ...
                <form onSubmit={submitCorrection}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Textarea id="fx-correction" rows={2} value={correcting.correction}
                      onChange={(e) => setCorrecting((c) => ({ ...c, correction: e.target.value }))} />
                  </div>
                  <div className="space-y-1.5">
        ...
        <Card className="panel-elevation">
          <CardHeader className="border-b border-[hsl(var(--border))] pb-4 flex flex-row items-start justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-[hsl(var(--muted-foreground))] mt-1">
                A completed action is not a working action. For a major nonconformity
                one must be verified effective before this closes.
              </p>
            </div>
            {!terminal ? (
              <Button size="sm" variant="outline" disabled={busy}
          ...
          <CardContent className="p-0">
            ...
            {actions.length === 0 ? (
              <p className="p-8 text-center text-[hsl(var(--muted-foreground))]">
                No corrective or preventive action has been raised against this finding.
              </p>
            ) : (
                      ...
                      <tr key={a.id} className="border-b border-[hsl(var(--border))] last:border-0">
                        ...
                        <td className="data-grid-td text-xs">
                          {a.effectiveness_verified === true ? (
                            <span className="text-[hsl(var(--success))] font-medium">Effective</span>
                          ) : a.effectiveness_verified === false ? (
                            <span className="text-[hsl(var(--destructive))] font-medium">
                              Not effective
                            </span>
                          ) : a.status === 'Complete' ? (
                            <span className="text-[hsl(var(--warning))]">Unverified</span>
                          ) : null}
                        </td>
            ...
            {checking ? (
              <form onSubmit={submitEffectiveness}
                ...
                <p className="text-sm font-medium">Effectiveness check: {checking.description}</p>
                <div className="space-y-1.5">
                  ...
                  <form onSubmit={submitVoid} className="space-y-3">
                    <div className="space-y-1.5">
                      ...
                      <Textarea id="vo-reason" rows={3} value={voidReason}
                        ...
                        placeholder="Voiding is the honest alternative to closing something that should never have been raised. A voided finding does not satisfy the critical-item rule on its audit." />
                    </div>
                    <div className="flex justify-end gap-2">
