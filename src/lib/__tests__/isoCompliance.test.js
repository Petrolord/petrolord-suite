...
describe('the vocabularies match the database', () => {
  it('has exactly the four finding types the gates key off', () => {
    expect(FINDING_TYPES).toEqual([
      'Major nonconformity', 'Minor nonconformity', 'Observation',
      'Opportunity for improvement',
    ]);
    expect(NONCONFORMITY_TYPES).toEqual(['Major nonconformity', 'Minor nonconformity']);
    expect(EFFECTIVENESS_REQUIRED_TYPES).toEqual(['Major nonconformity']);
  });

  it('has the five clause statuses and the six audit statuses', () => {
    expect(CLAUSE_STATUSES).toHaveLength(5);
    expect(AUDIT_STATUSES).toHaveLength(6);
    expect(CLAUSE_STATUSES).toContain('Not assessed');
  });

  it('never uses the word Compliant, which was the old app\'s whole vocabulary', () => {
    expect(CLAUSE_STATUSES).not.toContain('Compliant');
    expect(CLAUSE_STATUSES).not.toContain('Non-Compliant');
  });
});
