...
- ~~**OWNER DECISION: Technical Report Autopilot's backend is gone.**~~
  **RESOLVED 2026-08-29: owner chose REBUILD, and it shipped the same
  ...
  now actually reach the writer. Detail: ProjectManagementAfeReportAutopilot-STATUS.md.
...
| D1 integration (engines #183-#194) | **BUILT 2026-09-15, PR OPEN** (branch feat/engines-batch-ec-d1-fc1) | **The repaired engines vendored into the Suite, and every app that reads a changed result adapted.** Five subtree pulls, each verified twice (the vendored tree equals the engines tree, and the changed path set equals the engines range; 44, 16, 9, 6 and 10 paths, nothing outside packages/engines, nothing deleted): 8af7af5 (EC6 #183, EC4 #184, EC5 #185, EC2 plus the shared irrContract.js and EC1-10 #186, screening skipIrr #187, FC1-0 separator and spacing #188), 3634a45 (#189 DCA test seed, #190 EC2-3 eight-point capex sweep and EC2-6 RRT uplift pool, #191 EC6 reserves check, sour service screen and end-of-life cost), e8afedc (#193 EPE engine 3.10.0), 01f054a (#192 EC4 exact ties and input refusals) and 709172f (#194 EC5 forecast flags, closing S-curve point and exact knapsack), so packages/engines is engines main across the whole batch. **EC6**: the flow assurance card showed "undefined Risk" once `level` was retired (those were the risk register's words for a different quantity) and now shows the hazard score with the trigger, the points and the hazards behind it; a scenario card screened on a partial capex says so and names the blank fields; an empty plan reports no calendar span rather than a zero-day window. **EC5**: the AFE dashboard CPI tile reads N/A with the reason where it used to read a flattering 1.00 with nothing spent, SPI distinguishes no budget from not started, the EAC trend no longer divides by a zero budget, and the cost item form caps progress at 100 percent and shows the engine's own refusal with Save disabled. Capital Portfolio Studio reads every project through a helper, because `projectEmv` throws on a refused pos while the table calls it during render; the refusals are listed before Run is pressed. **EC2**: the summary table called `s.irr.toFixed(1)` and crashed on the null the IRR contract now returns for most regimes (the 25 year tail changes sign more than once), so it prints n/a with the reason, naming every in-band root and a root above the band; the deprecated `effectiveTaxRate` alias is not read at all; a tier table with a repeated threshold is refused by the engine and the refusal appears under the table being edited. **EC4**: the neutral VOI verdict, the single rounding behind card and verdict, and the accepted edge sums are gated. **EC6-1, EC6-3 and EC6-8 (#191)**: the FDP generation panel shows the reserves check (profile volume against the oil P50, wells implied against wells carried) and reads "Ready with warnings" rather than clean while they stand; the flow assurance card screens corrosion on the H2S partial pressure against the NACE MR0175 threshold instead of any H2S above zero, and says whether H2S is unmeasured, needs a pressure, is below the threshold or is sour service; the plan's end-of-life cost (an ABEX cost item, else the facility decommissioning estimate) is charged in the final production year of the plan economics and of every scenario card, which is named with its source, and the rate of return that legitimately becomes null there prints its reason. **EC2-3 and EC2-6 (#190)**: the capex sweep is eight points ending at 1.5 and the resource rent tax uplift sizes a one-time pool, both stated in the Designer help guide. **EC4-1 and EC4-4 (#192)**: every user-facing recommendation reads the card-precision tie set, so the Decision Tree Builder, the Decision Studio brief and the VOI Analyzer guidance say the decision is indifferent and name the tied branches instead of recommending one of two branches under a decision advantage of 0.00; a cleared cost or payoff box omits the field (0 by contract) rather than storing a blank the engine refuses, and the refusals name the node. **EPE v3.10 (#193)**: the Results viewer prints why there is no IRR and the roots behind it, adds the profitability index and the PSC cost and CIT allowance outstanding at cessation, and shows the new row fields only for runs that carry them; the Monte Carlo panel states what a share of iterations with no IRR means; run comparison warns, naming the engine version on each side, when one run stores its lines at field level and the other at the working-interest share. **EC5-1, EC5-9b and the exact knapsack (#194)**: a forecast entered below the money already spent and committed is kept and flagged on its line, with the flagged-line counts beside the estimate at completion, and a negative forecast is reported as ignored; the S-curve closes at the window end so an overrun cannot draw as an underrun; the portfolio answer is exact, so the results card, the help guide and the Decision Studio brief drop the grid resolution and the overshoot claim, and the case that used to overshoot by 2 funds 5,995 of 6,000. **Owner step after merge**: redeploy epe-cash-flow-engine, epe-cash-flow-engine-batch and epe-monte-carlo, which bundle the vendored engine through supabase/functions/_shared/epe-engine.ts. **FC1-0**: the separator studio wraps every engine call and names the refused input, passes an undefined K override rather than 0, asks for the water droplet in oil and the oil droplet in water separately (a study saved with the retired single size carries into both), selects through `preferred` with its status, and shows the one retention length, the interface height and both layers; the layout adapter keeps the engine's skip list beside its own, takes completeness from the engine flag, and the panel handles a null pass, both worst rankings and the shortfall fraction. Suite tests over every changed module (every adaptation with a negative control), full jest green against the known load flakes, build clean. |
| EC7 (PIA 2021 compliance) | **BUILT 2026-09-26, PR OPEN** (branch fix/epe-pia-2021-compliance; engines #262 MERGED 3778451, engines 3.12.0; migration 20260926150000 HELD) | **The PIA regime on the gazetted texts by default (owner decision D1), re-vendored into the Suite.** Engines #259, #260 and #261 vendored first so the pin moves to 3778451 (guard: 1029 paths, 0 deviations). The worked example fixture carries `pia_legacy_pre_audit: true` and keeps pinning the pre-audit engine (NPV 135,185,570.34); the default path is pinned at NPV 141,236,909.83 with 27 line items from the text oracle, plus framework per year, 4 USD/bbl allowance after the cap, deep offshore tranches and the six refusals. Held migration adds the nine compliance inputs, drops the TET and deep offshore defaults, and stamps every saved PIA config legacy. Run Console, Results Viewer, Case Detail and Fiscal Regime Designer carry the new inputs, the legacy toggle and notice, `pia_only_then_nta_2025`, `pia_notes`, the new row columns, the refusal fixes and the new template types. Ekene economics kit regenerated (field NPV10 1.98 MM). **Owner order:** apply the migration, redeploy epe-cash-flow-engine, epe-cash-flow-engine-batch and epe-monte-carlo, then upload the Suite. Status: docs/scope/EPE-STATUS.md. |

## 2026-09-23: four defects found by the Ekene demo kit economics build

Fixed on branch fix/economics-app-defects, with engines #244 (EPE 3.11.0).

- **EPE Run Console, simple escalation** said inflation drives the oil, gas,
  condensate and opex escalators with capex nominal, but the run payload sent
  the per-stream fields as they stood (3 percent defaults whatever inflation
  said). `epeEscalation.payloadEscalators` now sends what the screen says; a
  saved configuration or assumption set whose escalators differ from its
  inflation opens in per-stream mode, and opening per-stream starts from the
  rates simple mode was using.
- **EPE breakeven with Apply economic limit test on** was null: at the
  bisection's low bound the limit trimmed every trailing year, capex years
  included. Engine 3.11.0 never trims a year carrying capital (engine and
  oracle; golden adds `elt_keeps_trailing_capex` and a breakeven case at
  48.416 USD/bbl that was null). **Needs the EPE edge-function redeploy**
  (/root/deploy_epe_functions.sh) to reach the server-side KPI.
- **IRR above the 1000 percent band** was always worded as a very high
  return, also when NPV at the discount rate is negative, where the high root
  is an artefact of the profile. EPE, NPV Scenario Builder, Fiscal Regime
  Designer and the FDP planner now say the project loses value in that case.
- **Probabilistic Breakeven** read the year of `2025-01-01` through
