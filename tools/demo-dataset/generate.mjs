...
// ===========================================================================
// 16. The rest of the Suite: the dynamic field, repackaged
// ===========================================================================
// packages/engines/test-data/ekene-dynamic/ already holds the field ON
// PRODUCTION, generated by the engines' own tools and carrying the goldens ten
// NextGen courses teach. Nothing is recomputed here: this section reshapes it
// into the files the Decline, Material Balance, SCAL and Waterflood apps read,
// so the story the first ten episodes tell carries into the next ten.

{
  ...
  // The surveillance rows are DAILY RATES despite the _bbl suffix: summing a
  // month of them and multiplying by the day count reproduces the ledger's Np
  // exactly. Checked here so the CSV headers can say so honestly.
  {
    const jan = flood.surveillance_rows.filter((r) => r.date === '2023-01-01');
    const sum = jan.reduce((a, r) => a + r.oil_bbl, 0);
    assertClose('surveillance rows are daily rates', sum * 31, flood.ledger_periods[0].Np, 1e-9);
  }

  const GAS_PER_OIL = LOCKED.rsi_scf_stb / 1000;   // mscf/d per bopd at Rs 400
  const byWell = new Map();
  const push = (well, date, oil, water, gas, inj) => {
    if (!byWell.has(well)) byWell.set(well, []);
    byWell.get(well).push({ date, oil, water, gas, inj });
  };
  // primary depletion, from the planted declines
  for (const w of rates.wells) {
    for (const m of w.monthly) {
      if (m.date >= LOCKED.flood_start) continue;
      push(w.name, m.date, m.oil_bpd, 0, m.oil_bpd * GAS_PER_OIL, 0);
    }
  }
  // the flood, from the surveillance ledger
  for (const r of flood.surveillance_rows) {
    push(r.well, r.date, r.oil_bbl, r.water_bbl, r.gas_mcf, r.inj_bbl);
  }

  const allRows = [];
  for (const [well, rows] of [...byWell].sort((a, b) => a[0].localeCompare(b[0]))) {
    rows.sort((a, b) => a.date.localeCompare(b.date));
    for (const r of rows) {
      allRows.push([well, r.date, n(r.oil, 6), n(r.water, 6), n(r.gas, 6), n(r.inj, 6)]);
    }
  }
  write('08-production/ekene-production-monthly.csv', csv(
    ['well', 'date', 'oil_rate_bopd', 'water_rate_bwpd', 'gas_rate_mscfd', 'injection_rate_bwpd'],
    allRows,
  ));

  // One file per producer, in the shape the Decline Curve app imports.
  for (const w of rates.wells) {
    const rows = byWell.get(w.name).map((r) => [r.date, n(r.oil, 6), n(r.water, 6), n(r.gas, 6)]);
    write(`08-production/decline/${w.name}.csv`, csv(
      ['date', 'oil_rate_bopd', 'water_rate_bwpd', 'gas_rate_mscfd'], rows,
    ));
  }
  ...
  // Material balance: the pressure survey history. Running it on this returns
  // the volumetric STOIIP, which is the integration fact worth filming.
  write('09-reservoir/ekene-mbal-tank-history.csv', csv(
    ['date', 'pressure_psia', 'cum_oil_stb', 'cum_gas_scf', 'cum_water_stb', 'bo_rb_stb', 'rs_scf_stb', 'bw_rb_stb'],
    mbal.inputs.production_data.map((d) => [d.observation_date, n(d.pressure_psia, 4), n(d.cum_oil_stb, 4),
      n(d.cum_gas_scf, 2), n(d.cum_water_stb, 4), n(d.bo_rb_stb, 6), n(d.rs_scf_stb, 2), n(d.bw_rb_stb, 4)]),
  ));
  write('09-reservoir/ekene-pvt-lab-table.csv', csv(
    ['pressure_psia', 'bo_rb_stb', 'rs_scf_stb', 'bw_rb_stb', 'oil_viscosity_cp'],
    mbal.inputs.pvt_lab_table.map((r) => [n(r.pressure_psia, 2), n(r.bo_rb_stb, 6), n(r.rs_scf_stb, 2),
      n(r.bw_rb_stb, 4), n(r.oil_viscosity_cp, 5)]),
  ));

  // Relative permeability: the locked Corey set, tabulated.
  {
    const { Swc, Sor, krwMax, kroMax, nw, no } = LOCKED.kr;
    const rows = [];
    for (let i = 0; i <= 40; i += 1) {
      const sw = Swc + ((1 - Sor - Swc) * i) / 40;
      const sn = (sw - Swc) / (1 - Swc - Sor);
      rows.push([n(sw, 6), n(krwMax * sn ** nw, 8), n(kroMax * (1 - sn) ** no, 8)]);
    }
    write('09-reservoir/ekene-relative-permeability.csv', csv(['sw', 'krw', 'kro'], rows));
  }

  // Capillary pressure: three lab plugs on three fluid systems, which collapse
  // onto one Leverett J curve — and that curve is what the logs' Sw came from.
  for (const plug of scal.capillary.lab_pc) {
    write(`09-reservoir/capillary/${plug.name}.csv`, csv(
      ['sw', 'pc_psi'], plug.rows.map((r) => [n(r.Sw, 6), n(r.Pc_psi, 6)]),
    ));
  }
  write('09-reservoir/capillary/plug-properties.csv', csv(
    ['plug', 'well', 'system', 'k_md', 'phi', 'sigma_dyncm', 'contact_angle_deg'],
    scal.capillary.lab_pc.map((p) => [p.name, p.well, p.system, n(p.sample.k_md, 1),
      n(p.sample.phi, 4), n(p.sample.sigma_dyncm, 1), n(p.sample.thetaDeg, 1)]),
  ));

  write('08-production/README.md', [
    '# The field on production', '',
    'Repackaged from `packages/engines/test-data/ekene-dynamic/`, which the engines',
    'generate and which ten NextGen courses teach off. Nothing here is recomputed.', '',
    `- Four producers (Ekene-1, -3, -5, -6) from ${LOCKED.production_start}, one per Arps model family.`,
    `- Two injectors (Ekene-2, -4) from ${LOCKED.flood_start}: both found the sand wet below the`,
    '  1560 m contact, which is why they became the injectors.',
    `- History ends ${LOCKED.history_end}.`, '',
    '**Every rate column is a daily rate** on the first of that',
    'month. The fixture\'s own surveillance rows carry a `_bbl` suffix on what are',
    'actually rates; the generator checks a month of them against the ledger before',
    ...
    'Material balance on the pressure history returns an OOIP of',
    `${mbal.expected.estimated_ooip_stb.toFixed(0)} stb at an R squared of ${mbal.expected.r_squared}.`,
    `The volumetric STOIIP from the mapped structure is ${LOCKED.stoiip_stb.toFixed(0)} stb.`,
    'Two completely different methods, one from a map and one from a pressure',
    'decline, landing on the same number. That is the demonstration.', '',
    'The capillary curves are the same ones the log saturations came from, so the',
    'Sw a petrophysicist computes in Episode 2 is the Sw the reservoir engineer',
    'assumes here.',
  ].join('\n'));

  say(`  production: ${allRows.length} well-months across ${byWell.size} wells, `
    + `${flood.ledger_periods.length}-month VRR ledger, `
    + `${mbal.inputs.production_data.length} pressure surveys, PVT, kr and ${scal.capillary.lab_pc.length} capillary plugs`);
  say(`  material balance on this history returns ${mbal.expected.estimated_ooip_stb.toFixed(0)} stb `
    + `against a volumetric ${LOCKED.stoiip_stb.toFixed(0)} stb`);
}
...
const EPISODES = [
  ...
  { n: 16, app: 'Seismolord Tops to Horizons', files: [
    ...
    + 'throw there and none at the Ekene Sand, so it is found below the reservoir, which is where the '
    + 'geology puts it. Track the framework, Accept, and the horizons come out named after their tops. For the prognosis beat, '
