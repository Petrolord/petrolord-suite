...
| **PP4 Restorable backup** (built 2026-09-02 client-side beside the offboarding dump; the server closure engine was not needed once §8.4 kept the dump as is, see STATUS) | "Back up my organization" and "Back up my work" doors on the Data Export page using the same writer with the largest root sets; multi-part packages; the offboarding dump stays untouched; restore = import into a new or empty org by an admin | Staging: export a disposable org, purge it with the existing offboarding path, restore into a fresh org, manifest counts match the restored counts table by table, blobs byte-identical |
| **PP5 Regulatory delivery** (built 2026-09-02: ECDSA P-256 signature rather than HMAC so verification works offline with an embedded public key; see STATUS) | Manifest HMAC signature; Certificate of Export PDF + public verify page (`/legal/verify-export`); README wording for regulators; per-package inventory page in-app | Certificate round trip and verify page; legal review of certificate wording (owner); a signed package altered after export is flagged on import |
