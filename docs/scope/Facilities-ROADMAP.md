...
| F5 | **SHIPPED 2026-08-29** (branch feat/facilities-f5) | Engines PR #81. Every predecessor defect replaced: z=0.85 → validated DAK; one K → published derating by pressure and mist extractor, floored honestly, vendor-overridable; assumed half-full → exact circular-segment geometry at the actual level; two-phase only → three-phase solving oil AND water retention against one vessel and naming the controller; no persistence → saved_separator_projects (APPLIED); one assumed L/D → the family swept and tabled; slug catcher gains the finger type and takes its volume from F1's pigging tab. Beyond the fixes: the dispersed-droplet check that catches the vessel meeting every retention target while still carrying water over. Oracle: segment areas by numerical chord integration, and Stokes from the SI law CHECKING the 1.78e-6 field constant (0.42 percent agreement). 20 gates, engines 1905 green. Suite: studio on the kit (slug kept), 2 tabs, help guide, smoke test; tile RENAME 20260829610000 HELD. Jest 369/5244 green; build clean. Detail: SeparatorStudio-STATUS.md. |
| F6 | **SHIPPED 2026-08-29** (branch feat/facilities-f6) | Engines PR #82. The predecessor was 76 LOC of nomogram equation times flat fudge factors (oil 0.1, scale 0.2) with NO velocity term, so a 4-inch and a 16-inch line answered identically. Now: DWM 1995 in resistance-in-series form (velocity and diameter enter explicitly; rate-against-velocity chart drawn), protective-scale correction above 60 C (hotter is not always worse), CO2 fugacity, wall shear against the ~100 Pa film-stripping threshold, inhibitor EFFICIENCY separated from AVAILABILITY (95 percent at 80 percent availability = 76 percent protection, stated), MR0175 regions from H2S AND pH with material guidance, H2S:CO2 regime check, and remaining life against an allowance (which the tile had advertised and the code never computed). Oracle: series combination by bisection, terms recomputed in natural logs vs log10, shear via Darcy vs Fanning, inhibitor as an explicit annual duty cycle. 19 gates, engines 1925 green. Suite: studio on the kit (slug kept), 3 tabs, help guide, smoke test; saved_corrosion_projects APPLIED; old corrosionCalculations.js + risk matrix deleted; tile RENAME 20260829630000 HELD. Jest 371/5265 green; build clean. Detail: CorrosionStudio-STATUS.md. |
| F7 | **SHIPPED 2026-08-29** (branch feat/facilities-f7) | Engines PR #83. The predecessor was a table of fixed removal efficiencies multiplied stage by stage, with temperature and TDS collected as inputs and NEVER USED in the math, Save/Export as toast stubs and a setTimeout(1000) pretending to compute. Now: oil in water is a log-normal DROPLET DISTRIBUTION and every device a GRADE-EFFICIENCY CURVE with a cut size, so the same device performs worse on finer water, three "90 percent" devices do NOT give 99.9 percent (each passes on what it cannot catch and the median falls down the train), and hot brine treats differently from cool fresh water because every cut runs on Stokes. Cut sizes from device physics: API 421 basins with the re-entrainment velocity limit, plate packs, hydrocyclones INCLUDING the turndown collapse when liners are starved, flotation with a residence floor, media filters with a loading limit. Oracle: the integral by MONTE CARLO vs binned quadrature, the CDF from the C library erf vs the A&S series, filtration marched layer by layer vs the closed exponential; the viscosity fit reproduces the textbook 0.890 cP at 25 C. 20 gates, engines 1946 green. Suite: studio on the kit (slug kept), 2 tabs, droplet chart with the cut lines drawn on it, help guide, smoke test; saved_pwt_projects APPLIED; usePwtCalculations.js deleted. Tile migration 20260829650000 (rename + ACTIVATION, the first non-rename of the series) HELD. Jest 373/5287 green; build clean. Detail: ProducedWater-STATUS.md. |
| F8 | **SHIPPED 2026-08-29** (branch feat/facilities-f8) | Engines PR #84. The mapper's tile had always advertised "safety distances" and the app computed none; this is that missing half, and it keeps two kinds of answer visibly apart. TABLE spacings between equipment classes are a table and say so (customary onshore figures, symmetric, replaceable by a site standard, null rather than a guess on an unknown pair). COMPUTED setbacks come from the duty: flare radiation on the same API 521 point source the F2 engine uses, pool fires on published burning-rate and Thomas flame-height correlations, so they MOVE when the duty moves and flag when the point source is being used inside the flame. Great-circle distances (a planar distance is wrong for a big site at latitude). Adapter refuses to judge pipe runs and custom icons and reports them instead of passing them silently. Equipment tags made sequential (the audit flagged Math.random tags: a tag is an identity a drawing and a datasheet share). Oracle: distance vs BOTH Vincenty and a 3D chord derivation, setbacks by round trip. 14 engine + 8 adapter gates, engines 1961 green. No tile migration: the tile's claim is simply true now. Jest 375/5310 green; build clean. Detail: LayoutMapper-STATUS.md. |
| F9 | **SHIPPED 2026-08-29** (branch feat/facilities-f9) | Engines PR #85. The first NEW app of the program (fresh slug `compressor-station-designer`; the F0-retired pack stays archived and redirecting per the no-revival doctrine). GPSA Ch.13: stage count from BOTH the ratio rule and the discharge-temperature limit with the governing one NAMED (temperature usually wins on hot or high-k gas, and a ratio rule alone under-stages exactly those); polytropic head with the exponent derived from the polytropic efficiency, not the isentropic exponent misused in its place (worth ~10 percent of power); Z at both ends of each stage, averaged; interstage cooling duty reported; recip-vs-centrifugal on the published criteria with "either" allowed; driver fuel taken out of the stream being compressed. **A TEST CAUGHT A REAL BUG**: the isentropic efficiency had a spurious factor putting it ABOVE the polytropic efficiency, which is impossible for compression; with the published relation both power routes now agree to 1e-12. Oracle: head by Simpson integration of int(v dp), temperature by a 100,000-step march, stage count by brute force, power through SI watts so the 33000 packaging is checked. 15 gates, engines 1977 green. Suite: studio on the kit, 3 tabs, help guide, smoke test; saved_compressor_projects APPLIED; tile SEED 20260829670000 HELD. Jest 377/5327 green; build clean. Detail: CompressorStation-STATUS.md. |
| F10-F12 | not started | |
