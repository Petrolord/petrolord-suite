 ...
 * (python3 tools/validation/fluid/genfixtures.py), committed at
...
} from '../../../engines/fluid/pr78.js';
import {
  solveRachfordRice, stabilityTest, flashPT,
} from '../../../engines/fluid/flash.js';
import { COMPONENTS, buildBipMatrix } from '../../../engines/fluid/components.js';
...
import { phaseBoundaries, saturationPressure, tracePhaseEnvelope } from '../../../engines/fluid/envelope.js';
import {
...
import { tunedMixtureWithPlusFraction } from '../../../engines/fluid/tuning.js';
import { tuneToLab, predictTargets } from '../../../engines/fluid/labTune.js';

const here = path.dirname(fileURLToPath(import.meta.url));
const dataDir = path.join(here, '../../../test-data/fluid');
const goldens = JSON.parse(fs.readFileSync(path.join(dataDir, 'goldens.json'), 'utf8'));
const nist = JSON.parse(fs.readFileSync(path.join(dataDir, 'nistVaporPressure.json'), 'utf8'));
...
{
  const lit = JSON.parse(fs.readFileSync(path.join(here, '..', '..', '..', 'test-data', 'fluid', 'literature-fixtures.json'), 'utf8'));
...
banner('CASE 13: C7+ characterization vs oracle transcription + n-alkane recovery');
{
  ...
  const charRef = JSON.parse(fs.readFileSync(path.join(dataDir, 'characterizationReference.json'), 'utf8'));
