...
const helpContent = [
  ...
  },
  {
    ...
    content:
      "An untuned equation of state is a screening tool: expect saturation pressure off by 5 to 15 percent and stock-tank gravity several API points heavy. The Lab tuning card fixes that against your PVT report. Enter any of: measured saturation pressure (with its temperature), and the separator test's total GOR, stock-tank API and Bo. Separator measurements are read against the stages you configured in the Separators tab, with the flash temperature and pressure taken as reservoir conditions, so set those to the lab test conditions first. If the lab stock tank ran warmer than 60 F, add it as an explicit final stage; the report's volumes are still on the standard 60 F basis. Tuning regresses four bounded properties of the C7+ fraction only (critical temperature and pressure multipliers, the methane interaction coefficient, and the volume shift); library components are never altered. The before and after table shows exactly how well each measurement is matched. Matching everything at once is a balance: total GOR and stock-tank API both pull on the stock-tank oil volume, so with inconsistent measurements the fit splits the difference and tells you so. Applied tuning flows into every compositional result, the phase envelope, the EOS black-oil table and the Pipeline Sizer handoff, and it saves with your project. Reset to untuned at any time.",
  },
  {
    id: 'saveload',
    icon: Save,
