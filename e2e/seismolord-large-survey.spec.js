...
test.describe(RUN ? 'Seismolord large-survey benchmark' : 'Seismolord large-survey benchmark (set SEIS_BENCH=1)', () => {
  ...
  for (const mode of ['before', 'after']) {
    test(`targets 2, 3 and 5 from the server bricks (${mode})`, async () => {
      ...
      if (mode === 'after') {
        ...
        // 6 s here. It needs Stream C's v4 display bricks (a quarter of the
        // bytes at full resolution, a sixteenth at level 2). What is
        // asserted is that it lands, inside the budget.
  ...
  test.describe('over a 10 Mbps link', () => {
    ...
    test('target 4: first inline from the server over 10 Mbps (v1 bricks)', async () => {
      ...
      // is about 330 s of transfer whatever the viewer does. It needs the
      // v4 display levels (Stream C). What Stream L owns and asserts: the
