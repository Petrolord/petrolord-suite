...
describe('the vocabularies match the database', () => {
  it('has the severities the closure rule keys off', () => {
    expect(NCR_SEVERITIES).toEqual(['Critical', 'Major', 'Minor', 'Observation']);
    expect(NCR_EFFECTIVENESS_REQUIRED).toEqual(['Critical', 'Major']);
    NCR_EFFECTIVENESS_REQUIRED.forEach((s) => expect(NCR_SEVERITIES).toContain(s));
  });

  it('has exactly the six plan statuses', () => {
    expect(PLAN_STATUSES).toHaveLength(6);
    PLAN_STATUSES.forEach((s) => expect(Object.keys(PLAN_TRANSITIONS_KEYS)).toContain(s));
  });
});
