...
describe('the vocabularies match the database', () => {
  it('has the seven statuses the gates key off', () => {
    expect(LESSON_STATUSES).toEqual([
      'Draft', 'Submitted', 'Validated', 'Published', 'Embedded', 'Archived', 'Superseded']);
    expect(isVisible(lesson({ status: 'Embedded' }))).toBe(true);
    expect(isVisible(lesson({ status: 'Validated' }))).toBe(false);
  });

  it('has no reusability rating of any kind', () => {
    // The old register showed High / Medium / Low on every row, typed
    // into a data file, and a "High Reusability: 89" dashboard tile.
    expect(APPLICABILITY_SCOPES).not.toContain('High');
    expect(APPLICABILITY_SCOPES).toEqual([
      'This asset', 'This discipline', 'This organization', 'Industry-wide']);
  });

  it('knows which targets are Suite registers', () => {
    expect(SUITE_TARGET_TYPES).toEqual(['Risk register', 'Management of change']);
    SUITE_TARGET_TYPES.forEach((t) => expect(TARGET_TYPES).toContain(t));
    expect(APPLICATION_OUTCOMES).toEqual(['Adopted', 'Adapted', 'Rejected']);
  });
});
