...
Last updated: 2026-09-22 (tester feedback: navigation, slice player, slice toggles, wells, stability)
...
Branch `feat/seismolord-picks-vs-surfaces` (Suite) + engines PR #7
(`Petrolord/petrolord-engines` `feat/seismolord-pick-surface-export`,
subtree-pulled):

- **Two first-class objects from a horizon.** A horizon row
  (`seismic_horizons`) is the INTERPRETATION — the il/xl pick lattice.
  Converting it ("Save as surface" in the export dialog) now persists
  the gridded result as a SURFACE in the shared `geo_surfaces`
  registry (`src/lib/surfacesRegistry.js`) with full provenance (app,
  volume, horizon, gridding/velocity/fault params) — NO new DDL: the
  cross-app registry, its RLS and the private `surfaces` bucket
  already exist (Mapping & Surface Studio G4). Converted surfaces are
  therefore also visible in Mapping & Surface Studio and org-shareable
  there.
- **Explorer Surfaces section** (between Horizons and Faults):
  registry rows whose provenance matches the active volume
  (`services/surfacesService.js` — `isVolumeSurface`), with per-row
  context menu: Export as XYZ / CPS-3 / ZMAP+ / Irap classic (re-export
  straight from the stored grid, no re-gridding) and Delete.
- **Pick export** (the interpretation itself, no gridding):
  `engines/seismolord/pickExport.js` — `picksToPickRows` labels live
  picks with REAL inline/crossline numbers (geometry min + step) and
  world X/Y through the measured survey affine; writers:
  - Charisma 3D horizon (9-token `INLINE : <il> XLINE : <xl> x y z`
    dialect Petrel imports; dialect cross-checked against the
    seismiqb/OpendTect loader family),
  - five-column `il xl x y z` points (generic column-mapped import),
  - bare `x y z` points.
  Nulls are SKIPPED (interpretation points carry no sentinel). Z sign
  toggle: negative-down (suite convention, default) or positive-down
  (Petrel/Charisma convention). Depth conversion shares the exact
  surface-export path (`resolveSampleToZ` in
  `services/surfaceWorkflow.js`: volume velocity model incl. layer-cake
  boundaries wins over the constant fallback).
- **Irap classic surface writer** (`writeIrapClassic` in
  `lib/gridding/surfaceExport.js`): `-996` header, x-fastest
  south-first body (our storage order), Irap's own 9999900 sentinel,
  rotation 0 (export grids are axis-aligned bboxes). Petrel and RMS
  import it natively; layout matched to equinor/xtgeo's writer.
- **Export dialog** now has an object selector (Surface (gridded) vs
  Horizon picks); surface path unchanged plus Irap format + "Save as
  surface" destination; picks path hides gridding-only controls.
- **Oracle + goldens** (validation-first): `extract_goldens.py` grew
  `write_irap` and `write_picks` — pick lattices from the analytic
  dome for `dome_ieee` (axis-aligned) and `dome_step` (azimuth 180,
  il/xl steps 2/3) ship as float32 blobs in
  `test-data/seismolord/picks/*_meta.json`, with committed Charisma /
  il-xl / xyz text files; `dome_surface_irap.dat` joins the surface
  goldens. Existing goldens regenerated byte-identically (no drift).
- **Tests**: engines `seismolord.pickexport.test.js` (dialect
  anchors); Suite `pickExport.test.js` (byte identity vs oracle, both
  fixtures), `surfacesService.test.js`, Irap byte test in
  `griddingExport.test.js`. Full repo run 187 suites / 2382 passed;
  build green.
- **Follow-ons (open)**: display stored surfaces in the Map window;
  import external surfaces/picks INTO Seismolord; SEG-Y horizon
  amplitude export.
