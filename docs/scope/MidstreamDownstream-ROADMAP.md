...
| DS9 | **SHIPPED 2026-08-29** (branch feat/downstream-ds9) | **Carbon Footprint & Abatement Studio** - the roll-up of a ledger the other apps already produce, NOT a second silo fed by its own spreadsheets once a year. **COMPUTED AND REPORTABLE ARE SEPARATE QUESTIONS**: an inventory can be complete arithmetic and still not be filable, and the app states the total, marks it not reportable, and names exactly which lines are the reason. Merging those two is how a working number reaches a regulatory return. **Factors are REGISTERED, not shipped** (value/unit/source/version/vintage) - an unsourced factor is accepted, because refusing would make a first pass impossible, but it is flagged and it blocks reportability. **No GWPs are shipped**: they differ between IPCC assessment reports by enough to move a methane-heavy inventory by a fifth (test asserts the AR4/AR5 ratio on identical measurements), so the set is the user's, carries its own label, and appears on every result; an undeclared set alone makes the inventory unreportable. **Combustion CO2 comes from the CARBON, not a factor** - every carbon atom into a burner leaves as CO2, so it is conservation of mass and needs no source document; carbon that escapes is counted as methane (the test proves 2 percent unburned carbon is far more than 2 percent of the impact), which is why the **flare destruction efficiency is required rather than assumed**. **Intensity refuses without a BOUNDARY** - per tonne charged and per tonne saleable are different numbers for the same plant - and states what it may be compared with. The MACC **annualises capital** over each measure's life (comparing a one-off capex to a recurring saving is what makes every measure look expensive), puts self-funding measures on the left, and **FLAGS MEASURES ACTING ON THE SAME SOURCE AS NOT ADDITIVE** with the cumulative curve called an upper bound - the error nearly every abatement spreadsheet makes - plus catches claims exceeding what a source emits. It deliberately does NOT resolve the overlap, because sequencing is an engineering judgement a solver would only guess at. The path counts each measure from its start year and **names the residual as unabated with no measure identified**, never a wedge labelled 'further measures'. 47 engine tests (green first pass), 15 page tests. Persistence 20260830020000 APPLIED; tile 20260830030000 HELD. Jest 416 suites / 6168 green; build clean. |
| DS10 | **SHIPPED 2026-08-29** (branch feat/downstream-ds10) | **Flare Gas to Value Studio** - the module's tenth app, its bridge back upstream, and the last of the program. Screens CNG, mini-LNG, liquids extraction and gas-to-power against **the gas that is actually there**: liquids content (gal/Mscf C3+) is **DERIVED** from composition and component liquid densities rather than read off a table, and CO2 is tracked separately from inerts because a liquefaction train cares about CO2 specifically. **Screening has THREE states, not two**: a requirement with no limit set is **unchecked, not passed** (treating it as passed is how a route clears screening nobody did), and a failure names which requirement, what the gas is, what the limit was and the shortfall - 'not feasible' is not an answer anyone can act on. Requirement limits ship UNSET, since a licensor's CO2 limit is a design choice not physical law. Recovery is required (100 percent assumed is the optimism that sinks these cases); capital scales by **DS4's power law**, imported not reimplemented; valuation is handed to the sanctioned engine. **THE CLAIM THIS APP EXISTS TO STOP**: that a flare's gross emission is the abatement (the engine now says the gross emission is the starting point, and the abatement depends on what the recovered product displaces). Recover the gas and somebody burns it, so the abatement is the difference against a stated counterfactual - and it is **not reliably smaller**: displacing diesel abates MORE than the flare emitted (the diesel is abated too), displacing the same gas abates exactly the flare, displacing nothing abates less and can go negative. **Three answers from one flare**, and no abatement is reported at all until the counterfactual is declared. Credits are not priced off a gross figure (a credit that cannot be substantiated cannot be issued), and 'does it need credits' is reported separately from 'what are they worth' - the question a bid turns on. Failed routes **stay in the bid table** with their failure named. **A REAL CORRECTION**: the engine header claimed the abatement is 'usually much smaller' than the gross emission; the test proved it can be larger, and both the doctrine and the warning text were corrected. Also verified: at 92 percent destruction the **methane slip carries 47 percent of the flare's CO2e** - which is why the destruction efficiency is required. 43 engine tests, 15 page tests; **route guard extended to assert all ten seeded tiles are routed**. Persistence 20260830040000 APPLIED; tile 20260830050000 HELD. Jest 418 suites / 6228 green; build clean. |
