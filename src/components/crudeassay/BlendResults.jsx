...
const BlendResults = () => {
  ...
  // blend can be read against its components rather than on its own. Each
  // crude's value is the engine's volumePercentAt, so the chart draws the
  // same curve the yields are cut from; where a crude's curve says nothing
  // the line has a gap rather than a flat invented segment.
  const chartRows = blendedCurve.map((point) => {
    ...
    inputs.crudes.forEach((c) => {
      const v = volumePercentAt(c.curve, point.temperatureF);
      row[c.name] = v === null ? undefined : v;
    });
    return row;
  });
    ...
    return (
      <div className="rounded-lg border border-amber-800/60 bg-amber-950/30 p-4 flex items-start gap-3 text-amber-200">
        ...
        <div>
          ...
          <p className="text-sm mt-1 opacity-90">{blend.error}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat label="Sulfur" value={p.sulfurWtPct === null ? 'n/a' : `${fmt(p.sulfurWtPct, 3)} wt%`} basis={basisOf('sulfurWtPct', 'mass basis')} />
        <Stat label="TAN" value={p.tanMgKohG === null ? 'n/a' : `${fmt(p.tanMgKohG, 2)}`} basis={basisOf('tanMgKohG', 'mg KOH/g, mass basis')} />
        <Stat label="Viscosity" value={p.viscosityCSt === null ? 'n/a' : `${fmt(p.viscosityCSt, 1)} cSt`} basis={blend.bases?.viscosityCSt} />
        <Stat label="Nitrogen" value={p.nitrogenWtPct === null ? 'n/a' : `${fmt(p.nitrogenWtPct, 3)} wt%`} basis={basisOf('nitrogenWtPct', 'mass basis')} />
        <Stat label="Nickel" value={p.nickelPpm === null ? 'n/a' : `${fmt(p.nickelPpm, 1)} ppm`} basis={basisOf('nickelPpm', 'mass basis')} />
        <Stat label="Vanadium" value={p.vanadiumPpm === null ? 'n/a' : `${fmt(p.vanadiumPpm, 1)} ppm`} basis={basisOf('vanadiumPpm', 'mass basis')} />
        <Stat
          label="Watson K"
          value={characterization?.watsonK ? fmt(characterization.watsonK, 2) : 'n/a'}
          basis="on the blend's interpolated 50 percent point, a screening basis"
        />
      </div>
        ...
        <ChartFrame height={340} exportFilename="crude-blend-tbp">
          <LineChart data={chartRows} margin={{ top: 12, right: 24, left: 8, bottom: 28 }}>
            ...
            {inputs.crudes.map((c, i) => (
              <Line
                key={c.id} type="linear" dataKey={c.name}
