...
export const LAYOUT_MAPPER_HELP_CONTENT = [
  ...
  },
  {
    ...
    content:
      'Every pair of placed standard equipment is compared against a spacing table by equipment class. Distances are great-circle distances measured centre to centre between the icon positions, so the check knows nothing about the footprint of the real equipment: allow for it yourself. The table holds customary onshore production-facility figures that have not yet been verified against the published literature, so treat a pass as a screening result and use your site or company standard for design. A pair the table gives as zero has no requirement, and when every pair on a layout is like that the panel says nothing was checked rather than reporting a pass. Pipe runs and custom icons are not checked, an item you have not placed cannot be measured, and the panel lists what it skipped and why. Violations are listed by shortfall in metres, largest first, and two rankings are named: the largest shortfall in metres and the largest shortfall as a fraction of its own requirement, which are often different pairs. A check that could not judge everything is marked incomplete with the reasons.',
  },
  {
    id: 'radiation',
    icon: Flame,
