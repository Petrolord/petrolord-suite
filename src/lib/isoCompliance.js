...
export const clauseCoverage = (
  ...
  const cutoff = new Date(today.getFullYear() - cycleYears, today.getMonth(), today.getDate());
...
export const summarise = (
  ...
  const coverage = clauseCoverage({ clauses, auditClauses, audits }, today);
...
/* ------------------------------------------------------------------ */
/* Tokens. CSS variables inside the app, hex on the white chart        */
/* surface (the chart template standard). The app this replaces drew   */
/* dark Recharts with its own hsl() literals.                          */
/* ------------------------------------------------------------------ */
