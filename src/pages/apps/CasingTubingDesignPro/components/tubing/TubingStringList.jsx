...
const TubingStringList = ({ selectedId, onSelect }) => {
  ...
  const handleDelete = (id, e) => {
    ...
    setStrings((prev) => ({
      ...prev,
      tubingStrings: prev.tubingStrings.filter((s) => s.id !== id),
    }));
  ...
  const handleDuplicate = (str, e) => {
    ...
    setStrings((prev) => ({
      ...
      tubingStrings: [...prev.tubingStrings, {
        ...
        id: `ts-${stamp}`,
        name: `${str.name} (Copy)`,
        sections: str.sections.map((s, i) => ({ ...s, id: `tsec-${stamp}-${i}` })),
      }],
    }));
  ...
  return (
    <div className="space-y-3">
      ...
      {tubingStrings.length > 1 && (
        <p className="text-[10px] text-amber-400 px-1">
          Load analysis runs on the first tubing string; extras are schematic only.
        </p>
      )}
