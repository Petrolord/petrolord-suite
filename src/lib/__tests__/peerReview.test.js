...
describe('rollup', () => {
  ...
  it('gives every value a token and every charted one a colour', () => {
    STAGES.forEach((s) => {
      expect(STAGE_TOKENS[s]).toMatch(/^--/);
      expect(STAGE_CHART_COLORS[s]).toMatch(/^#/);
    });
    SEVERITIES.forEach((s) => expect(SEVERITY_CHART_COLORS[s]).toMatch(/^#/));
    COMMENT_STATUSES.forEach((s) => expect(COMMENT_STATUS_TOKENS[s]).toMatch(/^--/));
  });
});

describe('GUARD: there is only one peer review authority', () => {
  const ROOT = path.resolve(__dirname, '../../..');
  const AUTHORITY = path.join('src', 'lib', 'peerReview.js');
  ...
  const APP = path.join(ROOT, 'src', 'pages', 'apps', 'assurance', 'peer-review');
  ...
  it('no file writes its own stage, priority or decision colour map', () => {
    // components/StatusBadges.jsx held three of them.
    const offenders = scanned.filter((f) =>
      /case\s+'(in review|verification|approved with conditions|closed)'/i.test(codeOf(f)));
    expect(offenders.map((f) => path.relative(ROOT, f))).toEqual([]);
  });

  it('no file decides a comment transition for itself', () => {
    const offenders = scanned.filter((f) => {
      const src = codeOf(f);
      return /status\s*===\s*['"]Responded['"]\s*\?/.test(src)
        || /setStatus\(['"]Verified['"]\)/.test(src);
    });
    expect(offenders.map((f) => path.relative(ROOT, f))).toEqual([]);
  });

  it('no file compares a due date to now on its own', () => {
    const offenders = scanned.filter((f) =>
      /new Date\((?:\w+\.)?due_date\)\s*<\s*new Date\(\)/.test(codeOf(f)));
    expect(offenders.map((f) => path.relative(ROOT, f))).toEqual([]);
  });

  it('every file in the app that derives something imports it from here', () => {
    const derives = scanned.filter(
      (f) => f.startsWith(APP) && /summarise|canClose|isOverdue|STAGE_TOKENS|canTransition/.test(codeOf(f)));
    expect(derives.length).toBeGreaterThan(0);
    derives.forEach((f) => expect(codeOf(f)).toMatch(/peerReview/));
  });

  it('the vocabularies match the database constraints added in AS5', () => {
    const migration = fs.readFileSync(
      path.join(ROOT, 'supabase/migrations/20260917300000_as5_peer_review.sql'), 'utf8');
    STAGES.forEach((s) => expect(migration).toContain(`'${s}'`));
    PRIORITIES.forEach((p) => expect(migration).toContain(`'${p}'`));
    DECISIONS.forEach((d) => expect(migration).toContain(`'${d}'`));
    SEVERITIES.forEach((s) => expect(migration).toContain(`'${s}'`));
    COMMENT_STATUSES.forEach((s) => expect(migration).toContain(`'${s}'`));
  });
});
