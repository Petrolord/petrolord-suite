      ...
      <div className="border-t border-slate-800 pt-3 space-y-2">
        ...
        {legacyDesigns.length === 0 ? (
          <Button size="sm" variant="outline" className="w-full h-8" onClick={loadLegacyDesigns}>
            <History className="w-3 h-3 mr-1" /> Look for old ESP inputs
          </Button>
        ) : (
        ...
        <p className="text-[11px] text-slate-600">
          The old designer's ESP tab staged about ten times short and was removed, but its inputs
          were kept. The well and duty numbers import; the pump model name does not, because those
          catalog entries were invented curves and there is nothing honest to map them to.
        </p>
      </div>
    </div>
  );
