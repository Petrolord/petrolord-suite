...
-- Optional: surface Fluid Studio projects in the central "My Projects" view.
-- Add this UNION branch to public.get_all_my_projects() (see
-- src/database/functions/get_all_my_projects.sql), then CREATE OR REPLACE it:
--
--   UNION ALL
--   SELECT id, project_name, created_at, 'fluid_studio' AS app_type,
--          results_data AS project_data, inputs_data, results_data
--   FROM public.saved_fluid_studio_projects WHERE user_id = auth.uid()
