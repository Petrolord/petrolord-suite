// Generic grid math shared across geoscience apps — extracted from
// Mapping & Surface Studio's engine/surface.js at the second consumer
// (Earth Modeling, G8.1), the WorkspaceShell/waveform/gridding rule.
// Pure functions, no I/O.
//
// Grid convention (matches src/lib/gridding): z is a row-major typed
// array of nx*ny, z[r*nx + c], with world x[c] = x0 + c*dx and
// y[r] = y0 + r*dy (row 0 = south). A spec may carry `rotation_deg`
// (MS5, 2026-09-06): see gridXY for the frame. NULL_VALUE (1e30) marks empty
// nodes. Outputs preserve the input array type (Float32Array in ->
// Float32Array out; Float64Array in -> Float64Array out), so callers
// choose their precision and the f32 house default is unchanged.

import { NULL_VALUE } from '../../lib/gridding/numeric';

export const isNull = (v) => !Number.isFinite(v) || Math.abs(v) >= 1e29;

const outArray = (like, length) =>
  new (ArrayBuffer.isView(like) ? like.constructor : Float64Array)(length);

/** Grid rotation in degrees (0 when the spec carries none). */
export const gridRotation = (spec) => (Number.isFinite(spec?.rotation_deg) ? spec.rotation_deg : 0);

const rotCS = (spec) => {
  const t = (gridRotation(spec) * Math.PI) / 180;
  return { cos: Math.cos(t), sin: Math.sin(t) };
};

/**
 * World coordinates of node (r, c) on a spec. A rotated spec
 * (`rotation_deg`, the Irap and Petrel convention: the grid's local X
 * axis turned anticlockwise from world east by that angle, origin at
 * node (0, 0)) places the node at the origin plus c*dx along local X
 * and r*dy along local Y. Unrotated specs use the plain arithmetic.
 */
export const gridXY = (spec, r, c) => {
  const rot = gridRotation(spec);
  if (rot === 0) return { x: spec.x0 + c * spec.dx, y: spec.y0 + r * spec.dy };
  const { cos, sin } = rotCS(spec);
  const lx = c * spec.dx;
  const ly = r * spec.dy;
  return { x: spec.x0 + lx * cos - ly * sin, y: spec.y0 + lx * sin + ly * cos };
};

/** Fractional node index {fx, fy} of a WORLD point (inverse of gridXY). */
export function worldToGridIndex(spec, x, y) {
  const rot = gridRotation(spec);
  const ex = x - spec.x0;
  const ey = y - spec.y0;
  if (rot === 0) return { fx: ex / spec.dx, fy: ey / spec.dy };
  const { cos, sin } = rotCS(spec);
  return { fx: (ex * cos + ey * sin) / spec.dx, fy: (-ex * sin + ey * cos) / spec.dy };
}

/** The four world corners of a spec, SW, SE, NE, NW in the local frame. */
export function gridCorners(spec) {
  return [gridXY(spec, 0, 0), gridXY(spec, 0, spec.nx - 1), gridXY(spec, spec.ny - 1, spec.nx - 1), gridXY(spec, spec.ny - 1, 0)];
}

/** Axis-aligned world bounding box of a spec. */
export function gridBBox(spec) {
  const cs = gridCorners(spec);
  return {
    xmin: Math.min(...cs.map((p) => p.x)), xmax: Math.max(...cs.map((p) => p.x)),
    ymin: Math.min(...cs.map((p) => p.y)), ymax: Math.max(...cs.map((p) => p.y)),
  };
}

/**
 * Bilinear sample of z (nx*ny) at fractional index (fx, fy). Outside
 * the frame, or with any null corner, returns NULL_VALUE. An exact
 * node hit needs only that node to be live.
 */
export function bilinearSample(z, nx, ny, fx, fy) {
  if (fx < 0 || fy < 0 || fx > nx - 1 || fy > ny - 1) return NULL_VALUE;
  const c0 = Math.floor(fx);
  const r0 = Math.floor(fy);
  const tx = fx - c0;
  const ty = fy - r0;
  const c1 = tx > 0 ? c0 + 1 : c0;   // only reach the next node when actually weighted
  const r1 = ty > 0 ? r0 + 1 : r0;   // so an exact node hit needs only that node
  const v00 = z[r0 * nx + c0];
  const v01 = z[r0 * nx + c1];
  const v10 = z[r1 * nx + c0];
  const v11 = z[r1 * nx + c1];
  if (isNull(v00) || isNull(v01) || isNull(v10) || isNull(v11)) return NULL_VALUE;
  return (v00 * (1 - tx) + v01 * tx) * (1 - ty) + (v10 * (1 - tx) + v11 * tx) * ty;
}

/** Bilinear sample at a WORLD coordinate on a spec. */
export function sampleAtXY(z, spec, x, y) {
  const { fx, fy } = worldToGridIndex(spec, x, y);
  return bilinearSample(z, spec.nx, spec.ny, fx, fy);
}

/**
 * Bilinear resample z (on specA) onto specB's frame. A target node
 * outside specA, or whose source neighbours include a null, becomes
 * null. Reproduces any bilinear (hence linear) field exactly.
 */
export function resampleTo(z, specA, specB) {
  const out = outArray(z, specB.nx * specB.ny);
  if (gridRotation(specA) === 0 && gridRotation(specB) === 0) {
    for (let r = 0; r < specB.ny; r++) {
      const wy = specB.y0 + r * specB.dy;
      const fy = (wy - specA.y0) / specA.dy;
      for (let c = 0; c < specB.nx; c++) {
        const wx = specB.x0 + c * specB.dx;
        const fx = (wx - specA.x0) / specA.dx;
        out[r * specB.nx + c] = bilinearSample(z, specA.nx, specA.ny, fx, fy);
      }
    }
    return out;
  }
  for (let r = 0; r < specB.ny; r++) {
    for (let c = 0; c < specB.nx; c++) {
      const w = gridXY(specB, r, c);
      const { fx, fy } = worldToGridIndex(specA, w.x, w.y);
      out[r * specB.nx + c] = bilinearSample(z, specA.nx, specA.ny, fx, fy);
    }
  }
  return out;
}

/** Elementwise a op b on the SAME spec; null if either side null. */
export function combine(zA, zB, op) {
  if (zA.length !== zB.length) throw new Error('Surfaces must share a grid frame — resample first.');
  const fn = { subtract: (a, b) => a - b, add: (a, b) => a + b, multiply: (a, b) => a * b }[op];
  if (!fn) throw new Error(`Unknown surface op "${op}".`);
  const out = outArray(zA, zA.length);
  for (let i = 0; i < zA.length; i++) {
    out[i] = (isNull(zA[i]) || isNull(zB[i])) ? NULL_VALUE : fn(zA[i], zB[i]);
  }
  return out;
}

/** Isochore = deeper − shallower (both positive-down => thickness). */
export const isochore = (zDeep, zShallow) => combine(zDeep, zShallow, 'subtract');

/** Thickness from two ELEVATION surfaces (negative below datum, the
 *  geo_surfaces convention): top − base, positive when the base is
 *  deeper. */
export const thickness = (zTopElev, zBaseElev) => combine(zTopElev, zBaseElev, 'subtract');

export const M_PER_FT = 0.3048;
const Z_UNITS = ['m', 'ft'];

/** Copy of z with live nodes converted between metres and feet; nulls
 *  preserved; same array type. */
export function convertZUnit(z, from, to) {
  if (!Z_UNITS.includes(from) || !Z_UNITS.includes(to)) throw new Error(`Unknown depth unit (expected m or ft, got "${from}" -> "${to}").`);
  const f = from === to ? 1 : from === 'ft' ? M_PER_FT : 1 / M_PER_FT;
  const out = outArray(z, z.length);
  for (let i = 0; i < z.length; i++) out[i] = isNull(z[i]) ? NULL_VALUE : z[i] * f;
  return out;
}

const vx = (v) => (Array.isArray(v) ? v[0] : v.x);
const vy = (v) => (Array.isArray(v) ? v[1] : v.y);

/** Even-odd test of a world point against a ring of [x, y] or {x, y}
 *  vertices (closing edge implied). */
function insideRing(ring, x, y) {
  const n = ring.length;
  let inside = false;
  for (let i = 0, j = n - 1; i < n; j = i, i++) {
    const xi = vx(ring[i]);
    const yi = vy(ring[i]);
    const xj = vx(ring[j]);
    const yj = vy(ring[j]);
    if ((yi > y) !== (yj > y) && x < ((xj - xi) * (y - yi)) / (yj - yi) + xi) inside = !inside;
  }
  return inside;
}

/**
 * Copy of z with every node whose centre lies outside the boundary
 * ring set to null (a clip to a lease, AOI or field outline). The ring
 * is in the grid's world frame.
 */
export function maskOutsidePolygon(z, spec, ring) {
  if (!Array.isArray(ring) || ring.length < 3) throw new Error('A boundary needs at least 3 vertices.');
  const out = outArray(z, z.length);
  for (let r = 0; r < spec.ny; r++) {
    for (let c = 0; c < spec.nx; c++) {
      const i = r * spec.nx + c;
      const w = gridXY(spec, r, c);
      out[i] = insideRing(ring, w.x, w.y) ? z[i] : NULL_VALUE;
    }
  }
  return out;
}

/** z + k on every live node. */
export function scalarAdd(z, k) {
  const out = outArray(z, z.length);
  for (let i = 0; i < z.length; i++) out[i] = isNull(z[i]) ? NULL_VALUE : z[i] + k;
  return out;
}

/** min / max / mean / live-node count over a grid. */
export function surfaceStats(z) {
  let min = Infinity;
  let max = -Infinity;
  let sum = 0;
  let n = 0;
  for (const v of z) {
    if (isNull(v)) continue;
    if (v < min) min = v;
    if (v > max) max = v;
    sum += v; n += 1;
  }
  return n ? { min, max, mean: sum / n, count: n } : { min: null, max: null, mean: null, count: 0 };
}
