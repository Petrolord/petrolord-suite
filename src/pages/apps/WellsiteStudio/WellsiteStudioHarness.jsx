...
// `?seed=reference` loads the Reference Well (spec section 43) for the
// performance gate; `?conflict=1` adds a competing office version of the Agbada call; `?offline=1`
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (searchParams.get('empty') !== '1') {
        const reference = searchParams.get('seed') === 'reference';
        const well = await seedWellsite(backend, { openingRecords: !reference });
        backend.__wellId = well.id;
        if (searchParams.get('conflict') === '1') await seedCompetingTop(backend, well);
        if (reference) { const t0 = performance.now(); const counts = await seedReferenceWell(backend, well); window.__wsReference = { ...counts, seedMs: Math.round(performance.now() - t0) }; }
