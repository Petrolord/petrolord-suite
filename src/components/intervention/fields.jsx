...
export const Row = ({ label, value, hint, accent = 'text-slate-100' }) => (
  <div className="flex items-baseline justify-between gap-3 py-1.5 border-b border-slate-800/60 last:border-0">
    <div>
      <p className="text-sm text-slate-300">{label}</p>
      {hint && <p className="text-[11px] text-slate-600">{hint}</p>}
    </div>
    <p className={`text-sm font-semibold tabular-nums whitespace-nowrap ${accent}`}>{value}</p>
  </div>
);

export const VERDICT_STYLE = {
  candidate: { label: 'Candidate', className: 'text-emerald-400' },
  consider: { label: 'Worth considering', className: 'text-sky-400' },
  marginal: { label: 'Marginal', className: 'text-amber-400' },
  blocked: { label: 'Ruled out', className: 'text-rose-400' },
  unknown: { label: 'Not enough data', className: 'text-slate-500' },
  no: { label: 'No', className: 'text-slate-500' },
};
