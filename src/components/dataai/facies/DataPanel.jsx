// Electrofacies Studio: pick the data (Data & AI D3).
//
// Two sources: several wells from the wells registry (Well Data Manager),
// read through the registry's own services with each well's interval logs,
// or a table uploaded in the browser with a column naming the well of every
// row and, optionally, a core facies column. Empty states say what is
// missing and where it comes from; nothing is filled in.
...
const WellsSource = () => {
  const { setTable } = useElectrofacies();
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const t = await loadFaciesWellsTable(chosenWells, pickedCurves);
    ...
    return (
      <div className="space-y-2">
        ...
        <Note testId="wells-empty">
          ...
          and core facies as interval logs, or upload a table here instead.
        </Note>
      </div>
    );
  }
  return (
    <div className="space-y-2">
      <p className="text-[11px] text-slate-400">Wells (one or more; kNN and CART hold out whole cored wells)</p>
      ...
      {curves ? (
        <div className="space-y-1 rounded border border-slate-800 p-2" data-testid="curve-list">
          <p className="text-[11px] text-slate-400">Curves to read (the logs, and a facies code curve if you have one)</p>
          {curves.map((c) => (
            <Toggle
              key={c.name}
...
const UploadSource = () => {
  ...
  const [faciesCol, setFaciesCol] = useState('');
  const [chosen, setChosen] = useState([]);
  ...
  const adopt = (t) => {
    ...
    const well = c.find((x) => x.kind === 'text' && /WELL|UWI|NAME/i.test(x.name)) || c.find((x) => x.kind === 'text');
    setGroupCol(well ? String(well.index) : '');
    const depth = c.find((x) => x.kind === 'number' && /^(DEPT|DEPTH|MD)/i.test(x.name));
    setDepthCol(depth ? String(depth.index) : '');
    const fac = c.find((x) => /FACIES|LITHO/i.test(x.name) && (!well || x.index !== well.index));
    setFaciesCol(fac ? String(fac.index) : '');
    setChosen(c.filter((x) => x.kind === 'number' && (!depth || x.index !== depth.index) && (!fac || x.index !== fac.index)).map((x) => x.index));
  };
  ...
  const logCols = cols.filter((c) => c.kind === 'number' && String(c.index) !== depthCol && String(c.index) !== groupCol && String(c.index) !== faciesCol);

  return (
    <div className="space-y-2">
      <label className="block text-[11px] text-slate-400">
        <span>CSV, TSV, TXT or Excel file, one row per depth sample</span>
        <input
          type="file"
      ...
      <Note>
        The file stays in your browser. It needs a column naming the well of each row. A core facies column (text or
        codes) is optional: without one the studio clusters, but cannot compare with the core or train kNN and CART.
        A saved run keeps the columns so it can be re-run.
      </Note>
      ...
      {parsed && parsed.rows.length ? (
        <div className="space-y-2 rounded border border-slate-800 p-2" data-testid="upload-mapping">
          ...
          <SelectField label="Core facies column" value={faciesCol} onChange={setFaciesCol} emptyLabel="None" testId="facies-col" options={cols.filter((c) => String(c.index) !== groupCol).map((c) => ({ value: String(c.index), label: `${c.name} (${c.kind})` }))} />
          <div className="space-y-1">
            <p className="text-[11px] text-slate-400">Log columns</p>
            {logCols.map((c) => (
...
const TableSummary = () => {
  ...
  return (
    <div className="space-y-1 rounded border border-slate-800 bg-slate-950/60 p-2 text-xs" data-testid="table-summary">
      ...
      {table.intervals ? (
        <p className="text-slate-400" data-testid="interval-kinds">
...
const DataPanel = () => {
  const { sourceKind, setSourceKind, reloadError, reloading } = useElectrofacies();
  return (
