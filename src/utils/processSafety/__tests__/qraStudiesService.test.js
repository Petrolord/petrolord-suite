/**
 * ps_qra_studies persistence (PS3): organization scoped, author kept.
 */
const mockCalls = [];
...
import { createQraStudiesService, NO_ORG_MESSAGE } from '@/utils/processSafety/qraStudiesService';

beforeEach(() => { mockCalls.length = 0; mockResult = { data: [], error: null }; });

describe('ps_qra_studies service', () => {
  it('lists only the current organization, newest first', async () => {
    ...
    const list = await createQraStudiesService(() => 'org-1').list();
    expect(mockCalls).toContainEqual(['from', 'ps_qra_studies']);
  ...
  it('saves under the organization and never sends the author, which the database owns', async () => {
    ...
    await createQraStudiesService(() => 'org-1').save('s1', { name: '  Unit 3  ', study: {} });
  ...
  it('refuses to save without an organization', async () => {
    await expect(createQraStudiesService(() => null).save('s1', { name: 'x' })).rejects.toThrow(NO_ORG_MESSAGE);
  });

  it('reports a delete the policy refused instead of claiming success', async () => {
    mockResult = { data: [], error: null };
    await expect(createQraStudiesService(() => 'org-1').remove('s1'))
      .rejects.toThrow(/Only the author of a study or an organization owner or admin/);
    mockResult = { data: [{ id: 's1' }], error: null };
    await expect(createQraStudiesService(() => 'org-1').remove('s1')).resolves.toEqual({ success: true });
  });
  ...
  it('opens a stamped row through the state versioning and returns its payload', async () => {
    ...
    const p = await createQraStudiesService(() => 'org-1').load('s1');
    expect(p).toEqual({ name: 'A', study: { scenarios: [] } });
    expect(mockCalls).toContainEqual(['eq', 'organization_id', 'org-1']);
  });
});

describe('ps_qra_studies identity', () => {
  it('has its own table, state kind and migration, apart from the LOPA and consequence studies', async () => {
    const svc = await import('@/utils/processSafety/qraStudiesService');
    expect(svc.QRA_STUDIES_TABLE).toBe('ps_qra_studies');
    expect(svc.QRA_STUDY_KIND).toBe('ps-qra-study');
    expect(svc.QRA_STUDIES_MIGRATION).toBe('20260921100000_ps3_qra_studies');
  });
});
