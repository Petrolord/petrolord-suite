...
/**
 * Parse a date-only value at LOCAL midnight.
 *
 * `new Date('2026-01-01')` is UTC midnight, which is 31 December in
 * every negative offset. Comparing that to a local `new Date()` makes
 * an obligation due today read as overdue for anyone west of Greenwich.
 * Dates in this register are calendar dates, not instants.
 */
...
/** Whole calendar days from `today` to `date`. Negative means past. */
...
/** ISO date-only string, for writing a Date back to a `date` column. */
