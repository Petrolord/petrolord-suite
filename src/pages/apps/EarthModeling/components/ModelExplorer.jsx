      ...
      <div className="pb-3" data-testid="em-explorer">
        ...
        {culturePolygons.map((cp) => (
          <div className={rowCls} key={cp.id} data-testid={`em-culture-row-${cp.name}`}>
            <Spline className="w-3 h-3 text-amber-400 shrink-0" />
            <span className="truncate flex-1">{cp.name}</span>
            <span className="text-[10px] text-slate-600">{cp.vertices.length} pts</span>
        ...
        {!culturePolygons.length && (
          <p className="px-2 text-[11px] text-slate-600">No fault polygons in the registry. Draw them in Mapping &amp; Surface Studio (Polygons, fault block).</p>
        )}
