// Well model inputs (left rail, Well Model tab). The same description a
// nodal model needs: trajectory, temperatures, fluid, inflow and
// completion. Gas lift design is only as good as the well it is
// designed against, so this is a first-class panel rather than a
// hidden set of constants.
...
const WellModelPanel = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {well.mode === 'vertical' ? (
        <Field label="Well depth (ft)" hint="Node depth: the perforations or the packer.">
          <NumberInput section="well" name="depthFt" />
        </Field>
      ) : (
        <Field label="Survey (md, inc, azi)" hint="One station per line. Valve depths are reported in TVD and converted back to measured depth for the traverse.">
          <Textarea
            rows={4}
      ...
      <div className="border-t border-slate-800 pt-3 space-y-3">
        ...
        {inflow.calMode === 'pi' && (
          <Field label="PI (stb/d/psi)"><NumberInput section="inflow" name="pi" step="0.1" /></Field>
        )}
        {inflow.calMode === 'qmax' && (
      ...
      <div className="border-t border-slate-800 pt-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <Field label="Wellhead pressure (psia)"><NumberInput section="completion" name="whp" /></Field>
          <Field label="Water cut (%)"><NumberInput section="completion" name="wctPct" /></Field>
        </div>
