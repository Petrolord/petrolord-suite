...
/**
 * AS9 — the states the old app could not tell apart.
 *
 * It could not tell them apart because it never asked. No page in
 * Lessons Learned issued a query: every one imported MOCK_LESSONS and
 * METRICS from src/utils/lessons-learned/mockData.js, so an
 * organization with no lessons, one with a full register and a
 * database that was down all rendered the same five lessons by the
 * same five invented authors, under the same seven literal numbers.
 */
...
/** Shown while migration 20260917700000 is unapplied. */
export const SchemaNotice = () => (
  <div className="mb-4 p-4 rounded-xl border border-[hsl(var(--warning))]/30 bg-[hsl(var(--warning))]/5 text-sm">
    <p className="font-medium">This database has no lessons learned schema yet.</p>
